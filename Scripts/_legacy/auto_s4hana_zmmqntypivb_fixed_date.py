...
while dt < df_final:
    ...
    print(f'Iniciando extração: {end_year}-{end_month}-{end_day}')
    # ECFTO
    saplogin()
    ...
    os.system('taskkill /f /im cmd.exe')
    os.system('taskkill /f /im notepad.exe')
    print(f'Dados gerados para: {end_year}-{end_month}-{end_day}')

    dt = dt + DateOffset(days=1)
    dt = dt.date()