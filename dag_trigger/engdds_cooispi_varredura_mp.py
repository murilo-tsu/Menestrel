...
def engdds_cooispi_varredura_mp_main():
    logging.info("---- INICIANDO PROCESSO: ENGDDS_COOISPI_VARREDURA_MP.PY ----")
    ...
    def extrair_cooispi_varredura_mp():
        ...
        caminho_arquivo = meta_arquivos['engdds_cooispi_varredura_mp.py']['path']
        nome_arquivo = f"{now.tm_year}_{meta_arquivos['engdds_cooispi_varredura_mp.py']['files']}"
    ...
    executar_com_retry(erros=erros, etapa="COOISPI_VARREDURA_MP", funcao=extrair_cooispi_varredura_mp, tentativas=3, intervalo=60)
    ...
    if erros:
        raise RuntimeError(
            "Ocorreram erros em uma ou mais extrações de COOISPI_VARREDURA_MP:\n"
            + "\n".join(erros)
        )

    logging.info("---- ENGDDS_COOISPI_VARREDURA_MP.PY finalizado com sucesso ----")
