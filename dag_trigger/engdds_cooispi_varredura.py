...
def engdds_cooispi_varredura_main():
    ...
    executar_com_retry(erros=erros, etapa="COOISPI_VARREDURA", funcao=extrair_cooispi_varredura, tentativas=3, intervalo=60)
    ...
    if erros:
        raise RuntimeError(
            "Ocorreram erros em uma ou mais extrações de COOISPI_VARREDURA:\n"
            + "\n".join(erros)
        )

    logging.info("---- ENGDDS_COOISPI_VARREDURA.PY finalizado com sucesso ----")
