...
def extracoes_incrementais():
    ...
    try:
        ...
        if t(6, 0) <= datetime.now().time() <= t(18, 0):
...
schedule.every().day.at("05:30").do(run_with_retry, task09, task_name="TEXT_INFO")
schedule.every().day.at("18:30").do(run_with_retry, task13, task_name="ESTOQUE_FULL")
