...
def engdds_estoque_main():
    ...
    try:
        ...
        primeira_data = date.today() # + timedelta(days=-10) <--> SEM TIMEDELTA DURANTE A SEMANA
        # Artifício <--> usar dt_comp para tirar fotos de estoque em diferentes datas SE necessário,
        ...
        for day in dt_comp:
            ...
            # Iterando através das plantas declaradas em BUKRS
            try:
                ...
                for i in bukrs:
                    ...
                    session.findById("wnd[0]/usr/ctxtSO_BUDAT-LOW").text = f"01.{f(dt.month)}.{dt.year}"
                    ...
                    with sap.export_watchdog(180):
                        ...
                        nome_arquivo = f"{dt.year}-{f(dt.month)}-{f(dt.day)} {meta_arquivos['engdds_estoque.py']['files'][3]}{i}{meta_arquivos['engdds_estoque.py']['sufixo_files']}"
