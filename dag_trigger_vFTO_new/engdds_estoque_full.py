...
def engdds_estoque_full_main():
    logging.info("---- INICIANDO PROCESSO: ENGDDS_ESTOQUE_FULL.PY ----")
    ...
    try:
        ...
        #if domingo:
        lookback_days = -50
        primeira_data = date.today() + timedelta(days=lookback_days)
        #else:
        ...
        for day in dt_comp:
            ...
            try:
                ...
                for i in bukrs:
                    ...
                    # 2026-09-02 :: BUG ENCONTRADO --> DO NOT CALC NÃO TICKADO POR PADRAO
                    session.findById("wnd[0]/tbar[1]/btn[17]").press()
                    session.findById("wnd[1]/usr/txtV-LOW").text = "/ENSURE_CALC"
                    session.findById("wnd[1]/tbar[0]/btn[6]").press()

                    session.findById("wnd[0]/usr/ctxtSO_BUKRS-LOW").text = f"{i}"
                    ...
                    with sap.export_watchdog(180):
                        ...
                        nome_arquivo = f"{dt.year}-{f(dt.month)}-{f(dt.day)} {meta_arquivos['engdds_estoque.py']['files'][0]}{i}{meta_arquivos['engdds_estoque.py']['sufixo_files']}"
